...
onMounted(async () => {
  try {
    dataArray.value = await useFetchData('/api/orders', '2021-01-01', '2025-03-31', page.value);
...
const fetchIncomes = async (page) => {
  dataArray.value = await useFetchData('/api/orders', '2021-01-01', '2025-03-31', page);
  filtratedDataArray.value = useFilterDataArray(filters, dataArray)
};
...
const { chartData } = useChartData(filtratedDataArray, 'discount_percent', 'Скидка процент')
</script>
